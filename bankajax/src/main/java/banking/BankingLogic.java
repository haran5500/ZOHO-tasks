...
//			Object object = className.getDeclaredConstructor().newInstance();
	...
	/*
	 * private void loadData() throws CustomException { Map<Long, CustomerData>
	 * customerMap = connector.getAllCustomers(); Map<Long, Map<Long, AccountData>>
	 * accountMap = connector.getAllCustomerAccounts();
	 * 
	 * cache.setData(customerMap, accountMap); }
	 */

	public void loadCustomerData() throws CustomException {
		if (cache.getAllCustomer().isEmpty()) {
			
			Map<Long, CustomerData> customerMap = connector.getAllCustomers();

			cache.setCustomerData(customerMap);
		}

	}

	public void loadAccountData() throws CustomException {
		if (cache.getAllCustomerAccounts().isEmpty()) {
			
			Map<Long, Map<Long, AccountData>> accountMap = connector.getAllCustomerAccounts();
			cache.setAccountData(accountMap);
		}
	}
	...
	public void withdrawAmount(long customerID, long accountNum, double amount) throws CustomException {
	...
	public void depositAmount(long customerID, long accountNum, double amount) throws CustomException {
		CheckerUtil.validateNumbeNegative(amount);
